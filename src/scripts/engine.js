...
//função que desenha o resultado do último duelo e atualiza o score
...
//seleciona uma carte aleatoria entre as disponiveis
...
//função que desenha o resultado do jogo
async function drawGameResult() {
    let gameResult = '';

    if (state.score.playerScore >= 3) {
        state.view.winLoseH1.innerText = 'Você';
        state.view.winLoseSpan.innerText = 'Ganhou!';
        gameResult = 'win';
    } else if (state.score.computerScore >= 3){
        state.view.winLoseH1.innerText = 'Você';
        state.view.winLoseSpan.innerText = 'Perdeu!';
        gameResult = 'lose';
    }
...
//função que reseta o jogo e a partida
async function resetDuel() {
    state.values.conditionClick = true;
    ShowHiddenButtonCardFieldsImages(false);

    //se o jogo acabou, reseta os dados do jogo e inicia uma nova partida
